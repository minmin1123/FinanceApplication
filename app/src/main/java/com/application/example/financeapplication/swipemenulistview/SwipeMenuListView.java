package com.application.example.financeapplication.swipemenulistview;
        ...
        switch (action){
            case MotionEvent.ACTION_DOWN:
                ...
                if (mTouchPosition == oldPos && mTouchView != null
                        && mTouchView.isOpen()) {
                    //当按下的item不是当前已经打开的item,则关闭已经打开的item,并返回false.不再响应down以后的事件
                    if (view instanceof SwipeMenuLayout) {
...
//如果滑动了一下没完全展现，就收回去，这时候mTouchView已经赋值，再滑动另外一个不可以swip的view会导致mTouchView swip ,所以要用位置判断是否滑动的是一个view
